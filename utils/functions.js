...
export function filtering(
	...
	const { title, genre, score, episodes } = config;
	...
	if (title || genre || score || episodes) {
		const filtered_arr = arr.filter((o) => {
			...
			if (score) {
				cond =
					cond &&
					Number(score[0]) <= Number(o.score) &&
					Number(o.score) <= Number(score[1]);
			}

			if (episodes) {
				cond =
					cond &&
					Number(episodes[0]) <= Number(o.episodes) &&
					Number(o.episodes) <= Number(episodes[1]);
			}
...
export function sortArr(arr = [], config = { key: null, type: null }) {
